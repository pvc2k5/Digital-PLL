#!/bin/bash
BASE_DIRS=(
    "openroad/blocks_method/OpenROAD-flow-scripts"
    "openroad/hierarchy_method/OpenROAD-flow-scripts"
)

FILES_TO_MOD=("cells_adders.v" "cells_latch.v")

for base_dir in "${BASE_DIRS[@]}"; do
    TARGET_DIR="./$base_dir/flow/platforms/gf180"
...
EOF
        ...
        # 2. Sửa các file verilog (9 -> 7)
        for file in "${FILES_TO_MOD[@]}"; do
            FILE_PATH="$TARGET_DIR/$file"
            if [ -f "$FILE_PATH" ]; then
                # Tạo backup nếu chưa có (ẩn lỗi nếu file đã tồn tại)
                [ ! -f "$FILE_PATH.bak" ] && cp "$FILE_PATH" "$FILE_PATH.bak" 2>/dev/null
                # Sửa file
                sed -i 's/9/7/g' "$FILE_PATH"
            fi
        done
    fi
done
